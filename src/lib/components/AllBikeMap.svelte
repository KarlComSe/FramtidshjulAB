<script lang="ts">
  ...
  const bikeStats = $derived({
    ...
    speedLimit: bikesList.filter(
      (bike) => bike.speedlimit && bike.speed !== undefined && bike.speed > bike.speedlimit
    ),
  ...
  onMount(() => {
    updateInterval = setInterval(() => {
      ...
      throttledStats.movingBikes = currentBikes.filter((bike) => bike.moving).length;
      throttledStats.totalBikes = bikes.size;
      throttledStats.syncingBikes = bikeSyncService.getRunningSyncs().length;
      throttledStats.speedLimitBikes = currentBikes.filter(
        (bike) => bike.speedlimit && bike.speed !== undefined && bike.speed > bike.speedlimit
      ).length;
      throttledStats.inSpeedZone = currentBikes.filter((bike) => bike.speedlimit).length;
    }, UPDATE_INTERVAL);
  });
...
<div class="mb-4 border-l-4 border-yellow-500 bg-yellow-100 p-4 text-yellow-700">
  ...
  <p>
    This page is intended for debugging purposes only. It consumes a significant amount of resources
    and <strong>WILL</strong> slow down your browser if there is a large number of bikes.
  </p>
</div>
<div>
  <Mapped />
...
<details class="mb-4" bind:open={isMovingOpen}>
  <summary class="cursor-pointer text-lg font-semibold">
    Moving bikes <span class="text-blue-500">{throttledStats.movingBikes}</span>
  </summary>
  {#if isMovingOpen}
    <div class="mt-2 grid grid-cols-1 gap-4">
      {#each bikeStats.moving as bike (bike.id)}
        <div class="rounded border p-4 shadow">
          ...
          {bike.id}
          <pre class="mt-2 rounded bg-gray-100 p-2">{bike.speed}</pre>
          <pre class="mt-2 rounded bg-gray-100 p-2">{bike.latitude?.toFixed(
              4
...
<details class="mb-4" bind:open={isSyncingOpen}>
  <summary class="cursor-pointer text-lg font-semibold"
    >Syncing bikes <span class="text-blue-500">{throttledStats.syncingBikes}</span></summary
  >
  {#if isSyncingOpen}
...
<details class="mb-4" bind:open={isAllBikesOpen}>
  <summary class="cursor-pointer text-lg font-semibold"
    >All bikes <span class="text-blue-500">{throttledStats.totalBikes}</span></summary
  >
  {#if isAllBikesOpen}
    <div class="grid grid-cols-1 gap-4">
...
<details class="mb-4" bind:open={isWithSpeedLimitOpen}>
  <summary class="cursor-pointer text-lg font-semibold">
    Bikes over speed limit <span class="text-blue-500">{throttledStats.speedLimitBikes}</span>
  </summary>
    ...
    <div class="mt-2 grid grid-cols-1 gap-4">
      {#each bikeStats.speedLimit as bike (bike.id)}
        <div class="rounded border p-4 shadow">
          ...
          {bike.id}
          <pre class="mt-2 rounded bg-gray-100 p-2">{bike.speed}</pre>
          <pre class="mt-2 rounded bg-gray-100 p-2">{bike.speedlimit}</pre>
          <pre class="mt-2 rounded bg-gray-100 p-2">{bike.latitude?.toFixed(
              4
            )}..., {bike.longitude?.toFixed(4)}...</pre>
        </div>
...
<details class="mb-4" bind:open={isOverSpeedLimitOpen}>
  <summary class="cursor-pointer text-lg font-semibold">
    Bikes in speed zone <span class="text-blue-500">{throttledStats.inSpeedZone}</span>
  </summary>
