...
export class BikeRentService {
    static async startBike(bikeId: string) {
        ...
        if (!ongoingRental) {
            console.log(ongoingRental)
            throw new Error(`Bike can only be started if it is in an ongoing rental / trip. Contact support....`);
        }

        if (localBike.renter === undefined || localBike.renter !== ongoingRental.renter || localBike.status !== BikeStatus.Rented) {
            this.updateBikeStoreRentedElements(bikeId, BikeStatus.Rented, ongoingRental.customer.username);
        }

        if (!tripStore.hasOngoingTrip(bikeId)) {
            tripStore.startNewTrip(ongoingRental.id, bikeId, ongoingRental.customer.username, localBike.latitude, localBike.longitude);
        }
    }
    ...
    private static updateBikeStoreRentedElements(bikeId: string, status: BikeStatus, renter: string | undefined) {
